...
`check-ui.mjs` 检查重复 id、JS 引用但已不存在的 id、唯一样式表引用、CSS 选择器使用情况。
`audit-css.mjs` 审计**单文件内两层（历史层 / V2 覆盖层）的冲突**：`css/style.css` 是
唯一样式表，内部以 `V2_LAYER_START` 标记切分——标记之前是历史基础层，之后是
V2 覆盖层（原 `css/ui-v2.css`，2026-09-18 并入）。脚本列出给定类在两层里的全部规则，
并标出「历史层设置、V2 层未显式复位」的属性——这些就是"多余横线""割裂色块"这类
问题的来源，必须逐条确认。新增样式一律写进 V2 覆盖层小节，不要叠加同名块。
